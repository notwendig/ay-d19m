	...
	*buffer = 0;
	...
	if (ep & op)
	{
		if(code && code != 12 && code != 13 && code < 15)
		{
			...
			snprintf(buffer, bsz, "R=%d, M=%d, K=\'%c\', L=%d", RES_OK, ay_d19m_mode, key, bits);
		}
		else
			snprintf(buffer, bsz, "R=%d, M=%d, D=%8.8X, L=%d", RES_DATAERR, ay_d19m_mode, code0, bits);
	}
	else
	{
		snprintf(buffer, bsz, "R=%d, M=%d, D=%8.8X, L=%d", RES_PARITY, ay_d19m_mode, code0, bits);
	}
	return strlen(buffer);
}
...
int fmt_K4W26BF(uint32_t code0, int bits, char *buffer, size_t bsz)
{
	...
	if (ep & op)
	{
		snprintf(buffer, bsz, "R=%d, M=%d, F=%d, D=%d, L=%d", RES_OK, ay_d19m_mode, facility, code, bits);
	}
	else
	{
		snprintf(buffer, bsz, "R=%d, M=%d, D=%8.8X, L=%d", RES_PARITY, ay_d19m_mode, code0, bits);
...
int fmt_K6W26BCD(uint32_t code0, int bits, char *buffer, size_t bsz)
{
	...
	if (ep & op)
	{
		snprintf(buffer, bsz, "R=%d, M=%d, D=%X, L=%d", RES_OK, ay_d19m_mode, code, bits);
	}
	else
	{
		snprintf(buffer, bsz, "R=%d, M=%d, D=%8.8X, L=%d", RES_PARITY, ay_d19m_mode, code0, bits);
...
{
	snprintf(buffer, bsz, "R=%d, M=%d, D=%8.8X, L=%d", RES_NOSUPORT, ay_d19m_mode, code0, bits);
	return strlen(buffer);
}
